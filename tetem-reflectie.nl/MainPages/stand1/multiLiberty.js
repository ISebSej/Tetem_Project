...
// SETTINGS3 of this demo :
const SETTINGS3 = {
  maxFaces: 4 //max number of detected faces
}

// some globalz:
var THREECAMERA3 = null
...
function create_libertyMaterial () {
  return new THREE.MeshLambertMaterial({
    ...
    alphaMap: new THREE.TextureLoader().load(
      './assets/libertyAlphaMapSoft512.png'
    ),
    transparent: true,
...
function init_threeScene3 (spec) {
  console.log(spec)
  const threeStuffs = JeelizThreeHelper3.init(spec, detect_callback)
  ...
  function add_faceMesh (threeFaceMesh) {
    ...
    threeStuffs.faceObjects.forEach(function (faceObject) {
      //display the cube for each detected face
  ...
  libertyLoader.load('./assets/liberty.json', function (libertyGeometry) {
    JeelizThreeHelper3.sortFaces(libertyGeometry, 'z', true)
    const libertyMesh = new THREE.Mesh(
      libertyGeometry,
      create_libertyMaterial()
    )
  ...
  new THREE.BufferGeometryLoader().load(
    './assets/libertyFaceMask.json',
    function (faceGeometry) {
      JeelizThreeHelper3.sortFaces(faceGeometry, 'z', true)
  ...
  // CREATE THE CAMERA
  THREECAMERA3 = JeelizThreeHelper3.create_camera()
...
} //end init_threeScene3()

// Entry point, launched by body.onload():
function main3 () {
  JEEFACEFILTERAPI3.init({
    canvasId: 'canvasOutput3',
    NNCpath: '../resources/jeelizFaceFilter/dist/', // path of NNC.json file
    maxFacesDetected: SETTINGS3.maxFaces,
    callbackReady: function (errCode, spec) {
      ...
      console.log('INFO: JEEFACEFILTERAPI3 IS READY')
      init_threeScene3(spec)
    }, //end callbackReady()

    // called at each render iteration (drawing loop):
    callbackTrack: function (detectState) {
      JeelizThreeHelper3.render(detectState, THREECAMERA3)
    } //end callbackTrack()
  }) //end JEEFACEFILTERAPI3.init call
} //end main()
