...
function build_maskMaterial2 (fragmentShaderSource, videoDimensions) {
    ...
    uniforms: {
      ...
      dxy: {
        value: new THREE.Vector2()
          .fromArray(dxy)
          .multiplyScalar(SETTINGS.blurEdgeSoftness)
      }
    }
  })
  return mat
...
function init_threeScene2 (spec) {
  ...
  THREESCENE0RENDERTARGET = new THREE.WebGLRenderTarget(
    spec.canvasElement.width,
    spec.canvasElement.height,
    filters
  )
  THREESCENE1RENDERTARGET = new THREE.WebGLRenderTarget(
    spec.canvasElement.width,
    spec.canvasElement.height,
    filters
  )
  THREESCENE2RENDERTARGET = new THREE.WebGLRenderTarget(
    spec.canvasElement.width,
    spec.canvasElement.height,
    filters
  )
  ...
  */
  ...
  celFragmentShaderLoader.load('./shaders/celFragmentShader.gl', function (
    fragmentShaderSource
  ) {
    _maskMaterial = build_maskMaterial2(fragmentShaderSource, [
      spec.canvasElement.width,
      spec.canvasElement.height
    ])
  })
  _threeLoadingManager.onLoad = function () {
    console.log(
      'INFO in demo_celFace.js: all 3D assets have been loaded successfully :)'
    )
  ...
  _quad2DGeometry.addAttribute(
    'position',
    new THREE.BufferAttribute(videoScreenCorners, 2)
  )
  _quad2DGeometry.setIndex(
  ...
  const faceBlurAlphaXmesh = new THREE.Mesh(
    _quad2DGeometry,
    build_blurMaterial(
      [1 / spec.canvasElement.width, 0],
      THREESCENE0RENDERTARGET.texture
    )
  )
  const faceBlurAlphaYmesh = new THREE.Mesh(
    _quad2DGeometry,
    build_blurMaterial(
      [0, 1 / spec.canvasElement.height],
      THREESCENE1RENDERTARGET.texture
    )
  )
  ...
  THREECAMERA2 = new THREE.PerspectiveCamera(
    SETTINGS.cameraFOV,
    aspecRatio,
    0.1,
    100
  )
} // end init_threeScene2()

// entry point:
function main2 () {
  JeelizResizer.size_canvas({
    canvasId: 'canvasOutput2',
...
function init_faceFilter2 (videoSettings) {
  JEEFACEFILTERAPI2.init({
    canvasId: 'canvasOutput2',
    NNCpath: '../resources/jeelizFaceFilter/dist/', // root of NNC.json file
    ...
    callbackReady: function (errCode, spec) {
      ...
      console.log('INFO: JEEFACEFILTERAPI2 IS READY')
      console.log(spec)
    ...
    callbackTrack: function (detectState) {
      ...
      if (ISDETECTED) {
        ...
        const tanFOV = Math.tan(
          (THREECAMERA2.aspect * THREECAMERA2.fov * Math.PI) / 360
        ...
        const x = xv * D * tanFOV
        const y = (yv * D * tanFOV) / THREECAMERA2.aspect
      ...
      THREERENDERER.render(THREESCENE0, THREECAMERA2, THREESCENE0RENDERTARGET)

      // second pass: add gaussian blur on alpha channel horizontally
      THREERENDERER.render(THREESCENE1, THREECAMERA2, THREESCENE1RENDERTARGET)

      // second pass: add gaussian blur on alpha channel vertically
      THREERENDERER.render(THREESCENE2, THREECAMERA2, THREESCENE2RENDERTARGET)

      THREERENDERER.render(THREESCENE, THREECAMERA2)
    } // end callbackTrack()
  }) // end JEEFACEFILTERAPI2.init call
} // end main()
